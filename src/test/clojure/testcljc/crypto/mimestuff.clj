...
(is (with-open [ inp (CU/rc-stream "com/zotohlabs/frwk/mime/mime.eml") ]
...
(is (with-open [ inp (CU/rc-stream "com/zotohlabs/frwk/mime/mime.eml") ]
      (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^comzotohlabscljc.crypto.stores.CryptoStore ROOTCS 
...
(is (with-open [ inp (CU/rc-stream "com/zotohlabs/frwk/mime/mime.eml") ]
      (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^comzotohlabscljc.crypto.stores.CryptoStore ROOTCS ^String (first (.keyAliases ^comzotohlabscljc.crypto.stores.CryptoStore ROOTCS)) HELPME)
               msg (RU/new-mimeMsg "" "" inp)
               mp (.getContent msg)
               cs (.getCertificateChain pke)
               pk (.getPrivateKey pke)
...
(is (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^comzotohlabscljc.crypto.stores.CryptoStore ROOTCS ^String (first (.keyAliases ^comzotohlabscljc.crypto.stores.CryptoStore ROOTCS)) HELPME)
