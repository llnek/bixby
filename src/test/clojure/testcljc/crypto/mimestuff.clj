...
(is (with-open [ inp (CU/ResStream "com/zotohlab/frwk/mime/mime.eml") ]
      (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^cmzlabclj.nucleus.crypto.stores.CryptoStore ROOTCS
...
(is (with-open [ inp (CU/ResStream "com/zotohlab/frwk/mime/mime.eml") ]
      (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^cmzlabclj.nucleus.crypto.stores.CryptoStore ROOTCS ^String (first (.keyAliases ^cmzlabclj.nucleus.crypto.stores.CryptoStore ROOTCS)) HELPME)
               msg (RU/NewMimeMsg "" "" inp)
               mp (.getContent msg)
               cs (.getCertificateChain pke)
               pk (.getPrivateKey pke)
...
(is (let [ ^KeyStore$PrivateKeyEntry pke (.keyEntity ^cmzlabclj.nucleus.crypto.stores.CryptoStore ROOTCS ^String (first (.keyAliases ^cmzlabclj.nucleus.crypto.stores.CryptoStore ROOTCS)) HELPME)
