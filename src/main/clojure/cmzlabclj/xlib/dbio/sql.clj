...
(ns ^{:doc ""
  ...
  (:use [cmzlabclj.xlib.util.meta :only [BytesClass CharsClass]]
        [cmzlabclj.xlib.util.str :only [AddDelim! nsb strim]]
        [cmzlabclj.xlib.util.io :only [ReadChars ReadBytes ]]
        [cmzlabclj.xlib.util.core
         :only
         [FlattenNil notnil? NowJTstamp nnz]]
        [cmzlabclj.xlib.dbio.core]
        [cmzlabclj.xlib.util.dates :only [GmtCal]])
...
(defn- make-sql ""
  ...
  ^cmzlabclj.xlib.dbio.sql.SQueryAPI
...
(defn MakeProc ""
  ...
  ^cmzlabclj.xlib.dbio.sql.SQLProcAPI
