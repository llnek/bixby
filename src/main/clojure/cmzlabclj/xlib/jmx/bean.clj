...
(ns ^{:doc ""
  ...
  (:use [cmzlabclj.xlib.util.meta
        ...
        [cmzlabclj.xlib.util.str :only [HasAny?]])
  ...
  (with-local-vars [props (transient {})
    ...
    (doseq [^Method mtd (seq mtds) ]
      (let [ptypes (.getParameterTypes mtd)
            ...
            ^cmzlabclj.xlib.jmx.bean.BPropInfo
    ...
    (let [rc (persistent! @props) ]
      (doseq [[k ^cmzlabclj.xlib.jmx.bean.BPropInfo v] rc ]
  ...
  (let [impl (MakeMMap)cz (.getClass obj)
    ...
    (reify DynamicMBean
      ...
      (getAttribute [_ attrName]
        (let [^cmzlabclj.xlib.jmx.bean.BPropInfo
              prop (get propsMap attrName)
              ^cmzlabclj.xlib.jmx.bean.BFieldInfo
