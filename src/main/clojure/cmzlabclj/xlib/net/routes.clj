...
(ns ^{:doc ""
  ...
  (:use [cmzlabclj.xlib.util.core :only [MubleAPI MakeMMap test-nestr]]
        [cmzlabclj.xlib.util.str :only [nsb nichts? hgl?]]
...
(defn- initRoute ""
  ...
  [^cmzlabclj.xlib.util.core.MubleAPI rc
   ^String path]
...
(defn- seek-route "Returns [routeinfo, matcher]."
  ...
  (if-not (nil? rts)
    (some (fn [^cmzlabclj.xlib.net.routes.RouteInfo ri]
...
(defn MakeRouteCracker "Create a url route cracker."
  ...
  ^cmzlabclj.xlib.net.routes.RouteCracker
  [routes]
