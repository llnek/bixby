...
(defn HandleStatic "Handle static file resource."
  ...
  (let [^File appDir (-> ^Container
        ...
        cfg (.getAttr ^czlab.tardis.core.sys.Elmt
                      src
                      :emcfg)
...
(defn ServeError "Reply back an error."
  ...
  [^czlab.tardis.core.sys.Elmt src
   ^Channel ch
   code ]
  ...
  (with-local-vars [ok true mp nil]
    ...
    (when @ok
      (let [^File appDir (-> ^Container
        ...
        (let [cfg (.getAttr ^czlab.tardis.core.sys.Elmt src :emcfg)
              ^czlab.tardis.io.core.EmitAPI co src
              ^czlab.tardis.io.core.WaitEventHolder
              w (MakeAsyncWaitHolder (MakeNettyTrigger ch evt co) evt)]
          (.timeoutMillis w (:waitMillis cfg))
          (.hold co w)
          (.dispatch co
                     evt
                     {:router "czlab.tardis.mvc.comms.AssetHandler"
                      :info info
                      :path @mp}))))
  ))
  ...
  (with-local-vars [ok true]
    (try
      ...
      (catch AuthError e#
        ...
        (ServeError src ch 403)))
    (when @ok
      (let [^czlab.tardis.io.core.EmitAPI co src
            cfg (.getAttr src :emcfg)
            pms (.collect ri mc)
...
(deftype AssetHandler [] WHandler
  ...
  (run [_  j _]
    (require 'czlab.tardis.mvc.comms)
