  ...
  (:require [czlab.xlib.net.routes :refer [MakeRouteCracker RouteCracker]]
            ...
            [czlab.xlib.util.core
             ...
             [try!
              ...
              notnil?
              ConvLong]]
            [czlab.tardis.io.webss :refer [MakeWSSession]]
...
(defn MakeNettyTrigger "Create a Netty Async Trigger."
  ...
  ^czlab.tardis.io.core.AsyncWaitTrigger
  [^Channel ch evt src]
...
(defn- makeWEBSockEvent ""
  ...
  [^czlab.tardis.io.core.EmitAPI co
   ^Channel ch
   ssl
   ^WebSocketFrame msg]
...
(defmethod IOESReifyEvent :czc.tardis.io/NettyIO
  ...
  [^czlab.tardis.io.core.EmitAPI co & args]
    ...
    (channelRead0 [c msg]
      (let [ch (-> ^ChannelHandlerContext c (.channel))
        ...
        (if (instance? HTTPEvent evt)
          (let [^czlab.tardis.io.core.WaitEventHolder
                w
