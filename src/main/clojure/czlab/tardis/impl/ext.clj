...
(ns ^{:doc ""
  ...
  (:use [czlab.tardis.io.core :rename {enabled? io-enabled?} ]
        [czlab.xlib.util.consts]
        [czlab.tardis.core.consts]
        [czlab.tardis.io.loops]
        [czlab.tardis.io.mails]
        [czlab.tardis.io.files]
        [czlab.tardis.io.jms]
        [czlab.tardis.io.http]
        [czlab.tardis.io.netty]
        [czlab.tardis.io.jetty]
        [czlab.tardis.io.socket]
        [czlab.tardis.mvc.filters]
        [czlab.tardis.mvc.ftlshim]
        [czlab.tardis.impl.dfts
         :rename {enabled? blockmeta-enabled?} ]
        [czlab.tardis.impl.misc]
...
(defn- maybeGetDBPool ""
  ...
  ^JDBCPool
  [^czlab.tardis.core.sys.Elmt co ^String gid]
...
(defn- makeAppContainer ""
  ...
  ^Container
  [^czlab.tardis.impl.dfts.PODMeta pod options]
  ...
  (let [pub (io/file (K_APPDIR options) DN_PUBLIC DN_PAGES)
    ...
    (with-meta
      (reify
        ...
        (start [this]
          (let [^czlab.tardis.core.sys.Rego
                srg (.getf impl K_SVCS)
            ...
            (cond
              ...
              (.start ^czlab.tardis.core.sys.CljAppMain main)
        ...
        (stop [this]
          (let [^czlab.tardis.core.sys.Rego
            ...
            (cond
              ...
              (.stop ^czlab.tardis.core.sys.CljAppMain main)
        ...
        (dispose [this]
          (let [^czlab.tardis.core.sys.Rego
            ...
            (cond
              (satisfies? CljAppMain main)
              (.dispose ^czlab.tardis.core.sys.CljAppMain main)
              (instance? AppMain main)
...
(defn MakeContainer "Create an application container."
  ...
  ^Container
  [^czlab.tardis.core.sys.Elmt pod]

  (let [^czlab.tardis.impl.dfts.PODMeta pm pod
...
(defmethod CompConfigure :czc.tardis.ext/Container
  ...
  [^czlab.tardis.core.sys.Elmt co props]
...
(defn- doCljApp ""
  ...
  [ctr opts ^czlab.tardis.core.sys.CljAppMain obj]
...
(defn- doJavaApp ""
  ...
  [^czlab.tardis.core.sys.Elmt ctr
   ^AppMain obj]
...
(defn- mkDftAppMain ""
  ...
  (reify czlab.tardis.core.sys.CljAppMain
    (contextualize [_ ctr] )
    (initialize [_])
    (configure [_ cfg] )
    (start [_] )
    ...
    (let [^Properties mf (.getAttr co K_MFPROPS)
      ...
      (let [svcs (:services env) ]
        (if (empty? svcs)
          ...
          (.reifyServices ^czlab.tardis.impl.ext.ContainerAPI co)))
