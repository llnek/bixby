  ...
  (:require [czlab.xlib.netty.discarder :refer [MakeDiscardHTTPD]]
            ...
            [czlab.xlib.util.core
             ...
             [test-nonil
              ...
              PrintMutableObj
              MakeMMap]]
            [czlab.tardis.impl.exec :refer [MakeExecvisor]]
  ...
  (SimPTask "Primodial"
    (fn [^Job j]
      (let [^czlab.xlib.util.core.Muble
        ...
        ;;(test-cond "conf file:exec-visor" (= cz "czlab.tardis.impl.Execvisor"))
...
(defn- loadRes "Look for and load the resource bundle."
  ...
  (SimPTask "LoadResource"
    (fn [^Job j]
      (let [^czlab.xlib.util.core.Muble
            ctx (.getLastResult j)
            rc (GetResource "czlab.tardis/etc/Resources"
                            (.getf ctx K_LOCALE))]
