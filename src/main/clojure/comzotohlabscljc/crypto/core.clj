...
(ns ^{ :doc ""
  ...
  (:use [comzotohlabscljc.util.core :only [Bytesify Try! notnil? juid GetClassname] ])
  (:use [comzotohlabscljc.util.dates :only [PlusMonths] ])
  (:require [comzotohlabscljc.util.mime :as mime
...
(defn GetPkcsStore "Create a PKCS12 key-store."
  ...
  (^KeyStore [^InputStream inp ^comzotohlabscljc.crypto.codec.Password pwdObj]
...
(defn MakeKeypair "Make a Asymmetric key-pair."
  ...
  (let [ kpg (doto (KeyPairGenerator/getInstance algo _BCProvider)
                   (.initialize ^long keylen (GetSRand))) ]
...
(defn MakeCsrReq "Make a PKCS10 - csr-request."
  ...
  (do
    ...
    (let [ ^KeyPair kp (MakeKeypair (nsb RSA) keylen)
           k (.getPrivate kp)
           u (.getPublic kp)
...
(defn- mkSSV1 ""
  ...
  ^bytes
  [^KeyStore ks ^KeyPair kp ^comzotohlabscljc.crypto.codec.Password pwdObj options]
...
(defn- mkSSV3Cert ""
  ...
  (let [ ^String dnStr (:dnStr options)
         ...
         prv (.getPrivate kp)
         pub (.getPublic kp)
         bdr (JcaX509v3CertificateBuilder. issuer (NextSerial)
...
(defn- mkSSV3 ""
  ...
  (let [ ^PrivateKey issuerKey (last issuerObjs)
         ^long keylen (:keylen options)
         ^KeyPair kp (MakeKeypair (.getAlgorithm issuerKey) keylen)
...
(defn NewSession "Creates a new java-mail session."
  ...
  (^Session [^String user ^comzotohlabscljc.crypto.codec.Password pwdObj]
...
(defn NewMimeMsg "Create a new MIME Message."
  ...
  (^MimeMessage [^String user ^comzotohlabscljc.crypto.codec.Password pwdObj]
  ...
  (^MimeMessage [^String user ^comzotohlabscljc.crypto.codec.Password pwdObj ^InputStream inp]
  ...
  (let [ inp (mime/MaybeStream obj) ]
    (if (nil? inp)
      (if (instance? Multipart obj)
        ...
        (throw (IOException. (str "Invalid content: " (GetClassname obj)))))
      (try
    ...
    (let [ ^MimeMultipart mmp mp
           ...
           rc (some (fn [^SignerInformation si]
                      (let [ c (.getMatches s (.getSID si))
                        ...
                        (if (empty? rc) nil rc)))
                    (seq sns)) ]
      (when (empty? rc) (throw (GeneralSecurityException. "Verify signature: no matching cert.")) )
...
(defmethod SmimeEncrypt :multipart
  ...
  (let [ gen (SMIMEEnvelopedGenerator.)
         ^X509Certificate xc cert
         mm (doto (NewMimeMsg) (.setContent mp))
         g (-> (JceKeyTransRecipientInfoGenerator. xc) (.setProvider _BCProvider)) ]
    (.addRecipientInfoGenerator gen g)
    (.generate gen mm
      (-> (JceCMSContentEncryptorBuilder. algo) (.setProvider _BCProvider) (.build)))))
...
(defn PkcsDigSig "SMIME sign some data."
  ...
  (let [ gen (CMSSignedDataGenerator.)
    ...
    (.addCertificates gen (JcaCertStore. cl))
    (let [ ^CMSTypedData cms (if (.isDiskFile xdata)
                       ...
                       (loop [ digest nil stop false ]
                         (if (or stop (not (.hasNext it)))
                           ...
                           (let [ bdr (-> (JcaSimpleSignerInfoVerifierBuilder.)
                                        (.setProvider _BCProvider))
                                  ^X509CertificateHolder hdr (.next it)
                                  ok (.verify si (.build bdr hdr))
                                  dg (if ok (.getContentDigest si) nil) ]
                             (if (notnil? dg) (recur dg true) (recur nil false)))))))
                    (seq sls)) ]
...
(defn DescCert "Return a object"
  ...
  ( ^comzotohlabscljc.crypto.core.CertDesc
    [^bytes privateKeyBits ^comzotohlabscljc.crypto.codec.Password pwdObj]
      (let [ pkey (ConvPKey privateKeyBits pwdObj) ]
        (if (nil? pkey)
            (->CertDesc nil nil nil nil)
...
(defn ValidPKey? "Validate this Private Key."
  ...
  (let [ pkey (ConvPKey keyBits pwdObj) ]
    (if (nil? pkey)
      false
