...
(defn read-bytes "Read bytes and return a XData."
  (^XData [^InputStream inp usefile] (slurp-bytes inp (if usefile 1 (com.zotohlabs.frwk.io.IOUtils/streamLimit))))
  (^XData [^InputStream inp] (slurp-bytes inp (com.zotohlabs.frwk.io.IOUtils/streamLimit))) )

(defn read-chars "Read chars and return a XData."
  (^XData [^Reader rdr] (slurp-chars rdr (com.zotohlabs.frwk.io.IOUtils/streamLimit)))
  (^XData [^Reader rdr usefile] (slurp-chars rdr (if usefile 1 (com.zotohlabs.frwk.io.IOUtils/streamLimit)))))
