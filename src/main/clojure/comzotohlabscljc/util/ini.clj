...
(use '[ comzotohlabscljc.util.files :only [file-read?] ])
(use '[ comzotohlabscljc.util.core :only [conv-bool conv-long conv-double] ])
...
(defmulti ^comzotohlabscljc.util.ini.IWin32Conf parse-inifile "Parse a INI config file." class)
...
(defn- getKV
  ^String [^comzotohlabscljc.util.ini.IWin32Conf cf s k err]
...
(defmethod parse-inifile String
  ^comzotohlabscljc.util.ini.IWin32Conf [^String fpath]
  (if (nil? fpath)
    nil
    (parse-inifile (File. fpath))))

(defmethod parse-inifile File
  ^comzotohlabscljc.util.ini.IWin32Conf [^File file]
  (if (or (nil? file) (not (file-read? file)))
    nil
    (parse-inifile (.toURL (.toURI file)))))

(defn- parseIniFile
  ^comzotohlabscljc.util.ini.IWin32Conf [^URL fUrl]
...
(defmethod parse-inifile URL
  ^comzotohlabscljc.util.ini.IWin32Conf [^URL fileUrl]
  (if (nil? fileUrl)
    nil
