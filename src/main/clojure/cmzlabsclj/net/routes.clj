...
(ns ^{ :doc ""
  ...
  cmzlabsclj.net.routes

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:use [cmzlabsclj.util.core :only [MubleAPI MakeMMap test-nestr] ])
  (:use [cmzlabsclj.util.str :only [nsb nichts? hgl?] ])
...
(defn- initRoute ""
  ...
  [^cmzlabsclj.util.core.MubleAPI rc
   ^String path]
...
(defn- mkRoute ""
  ...
  [stat path ^cmzlabsclj.util.ini.IWin32Conf cfile]
...
(defn- seek-route ""
  ...
  (if-not (nil? rts)
    (some (fn [^cmzlabsclj.net.routes.RouteInfo ri]
...
(defn MakeRouteCracker "Create a url route cracker."
  ...
  ^cmzlabsclj.net.routes.RouteCracker
  [routes]
