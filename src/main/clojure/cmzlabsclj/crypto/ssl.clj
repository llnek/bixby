...
(ns ^{ :doc ""
  ...
  cmzlabsclj.crypto.ssl

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:use [cmzlabsclj.crypto.stores :only [MakeCryptoStore] ])
  (:use [cmzlabsclj.util.core :only [NewRandom] ])
...
(defn MakeSslContext "Make a server-side SSLContext."
  ...
  (^SSLContext [^URL keyUrl ^cmzlabsclj.crypto.codec.Password pwdObj]
