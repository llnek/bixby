...
(ns ^{ :doc ""
  ...
  (:use [cmzlabsclj.util.core :only [ThrowBadArg] ])
  (:use [cmzlabsclj.util.str :only [hgl?] ])
...
(defprotocol CryptoStore
  ...
  (addKeyEntity [_ ^bytes bits ^cmzlabsclj.crypto.codec.Password pwdObj] )
  (addCertEntity [_ ^bytes bits] )
  (trustManagerFactory [_] )
  (keyManagerFactory [_] )
  (certAliases [_] )
  (keyAliases [_] )
  (keyEntity [_ ^String nm ^cmzlabsclj.crypto.codec.Password pwdObj] )
...
(defn MakeCryptoStore ""
  ...
  ^cmzlabsclj.crypto.stores.CryptoStore
  [^KeyStore keystore ^cmzlabsclj.crypto.codec.Password passwdObj]
  ...
  (reify CryptoStore
    ...
    (addKeyEntity [this bits pwdObj]
      ...
      (let [ ch (.toCharArray ^cmzlabsclj.crypto.codec.Password pwdObj)
    ...
    (keyEntity [_ nm pwdObj]
      (let [ ca (.toCharArray ^cmzlabsclj.crypto.codec.Password pwdObj) ]
