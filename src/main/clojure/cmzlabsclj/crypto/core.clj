...
(ns ^{ :doc ""
  ...
  cmzlabsclj.crypto.core

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:require [clojure.math.numeric-tower :as math])
  (:use [cmzlabsclj.util.io :only [Streamify MakeBitOS ResetStream!] ])
  (:use [cmzlabsclj.util.seqnum :only [NextInt] ])
  (:use [cmzlabsclj.util.dates :only [PlusMonths] ])
  (:use [cmzlabsclj.util.core
         :only
         [ThrowIOE ThrowBadArg ternary NewRandom
          Bytesify Try! notnil? juid GetClassname] ])
  (:require [cmzlabsclj.util.mime
             :as mime
...
(defn- regoCerts ""
  ...
  [ ^KeyStore ks
    ^cmzlabsclj.crypto.codec.Password pwdObj]
...
(defn GetPkcsStore "Create a PKCS12 key-store."
  ...
  (^KeyStore [^InputStream inp ^cmzlabsclj.crypto.codec.Password pwdObj]
...
(defn- mkSSV1 ""
  ...
  [^KeyStore ks ^KeyPair kp
   ^cmzlabsclj.crypto.codec.Password pwdObj options]
...
(defn MakePkcs12 "Make a PKCS12 object from key and cert."
  ...
  [^bytes keyPEM ^bytes certPEM
   ^cmzlabsclj.crypto.codec.Password pwdObj ^File out]
...
(defn MakeSSv1PKCS12 "Make a SSV1 (root level) type PKCS12 object."
  ...
  [^String dnStr ^cmzlabsclj.crypto.codec.Password pwdObj
   ^File out options]
...
(defn- mkSSV3 ""
  ...
  [ ^KeyStore ks ^cmzlabsclj.crypto.codec.Password pwdObj
    issuerObjs options ]
...
(defn ExportPkcs7 "Extract and export PKCS7 info from a PKCS12 object."
  ...
  [^URL p12File ^cmzlabsclj.crypto.codec.Password pwdObj
   ^File fileOut]
...
(defn NewSession "Creates a new java-mail session."
  ...
  (^Session [^String user ^cmzlabsclj.crypto.codec.Password pwdObj]
...
(defn NewMimeMsg "Create a new MIME Message."
  ...
  (^MimeMessage [^String user ^cmzlabsclj.crypto.codec.Password pwdObj]
...
(defn DescCertificate "Get some basic info from Certificate."
  ...
  ^cmzlabsclj.crypto.core.CertDesc
...
(defn DescCert "Return a object"
  ...
  (^cmzlabsclj.crypto.core.CertDesc [^bytes privateKeyBits ^cmzlabsclj.crypto.codec.Password pwdObj]
    (if-let [ pkey (ConvPKey privateKeyBits pwdObj) ]
      (DescCertificate (.getCertificate pkey))
      (->CertDesc nil nil nil nil)))

  (^cmzlabsclj.crypto.core.CertDesc [^bytes certBits]
...
(defn ValidPKey? "Validate this Private Key."
  ...
  [^bytes keyBits ^cmzlabsclj.crypto.codec.Password pwdObj]
