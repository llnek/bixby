...
(ns ^{ :doc ""
  ...
  cmzlabsclj.tardis.io.mails

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:use [cmzlabsclj.crypto.codec :only [Pwdify] ])
  (:use [cmzlabsclj.util.seqnum :only [NextLong] ])
  (:use [cmzlabsclj.util.core :only [ThrowIOE TryC notnil?] ])
  (:use [cmzlabsclj.util.str :only [hgl? nsb] ])
...
(defn- read-pop3 ""
  ...
  [^cmzlabsclj.tardis.io.core.EmitterAPI co msgs]

  (let [^cmzlabsclj.tardis.core.sys.Element src co]
