...
(ns ^{ :doc ""
  ...
  cmzlabsclj.tardis.impl.ext

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:require [clojure.data.json :as json])
  (:use [cmzlabsclj.tardis.core.constants])
  (:use [cmzlabsclj.tardis.io.core :rename {enabled? io-enabled?} ])
  (:use [cmzlabsclj.tardis.io.loops])
  (:use [cmzlabsclj.tardis.io.mails])
  (:use [cmzlabsclj.tardis.io.files])
  (:use [cmzlabsclj.tardis.io.jms])
  (:use [cmzlabsclj.tardis.io.http])
  (:use [cmzlabsclj.tardis.io.netty])
  (:use [cmzlabsclj.tardis.io.socket])
  (:use [cmzlabsclj.tardis.mvc.handler])
  ;;(:use [cmzlabsclj.tardis.io.events])
...
(defn- make-jobcreator ""
  ...
  ^cmzlabsclj.tardis.impl.ext.JobCreator
  [parObj]
    ...
    (with-meta
      (reify
        ...
        (notifyObservers [this evt options]
          (let [ ^cmzlabsclj.tardis.impl.ext.JobCreator
        ...
        (start [this]
          (let [ pub (File. (.getAppDir this) (str DN_PUBLIC "/" DN_PAGES))
            ...
            (cond
              ...
              (.start ^cmzlabsclj.tardis.impl.ext.CljAppMain main)
              (instance? AppMain main) ;; java app
              (.start ^AppMain main)
              :else nil)))

        (stop [this]
          (let [ ^cmzlabsclj.tardis.core.sys.Registry
                 srg (.getf impl K_SVCS)
            ...
            (cond
              ...
              (.stop ^cmzlabsclj.tardis.impl.ext.CljAppMain main)
        ...
        (dispose [this]
          (let [ ^cmzlabsclj.tardis.core.sys.Registry
            ...
            (cond
              (satisfies? CljAppMain main)
              (.dispose ^cmzlabsclj.tardis.impl.ext.CljAppMain main)
              (instance? AppMain main)
...
(defn MakeContainer ""
  ...
  (let [ c (make-app-container pod)
         ...
         ^URL url (.srcUrl ^cmzlabsclj.tardis.impl.defaults.PODMeta pod)
         ps { K_APPDIR (File. (.toURI  url)) K_APP_CZLR cl } ]
    (CompCompose c apps)
    (CompContextualize c ctx)
    (CompConfigure c ps)
    (if (.enabled? ^cmzlabsclj.tardis.impl.ext.ContainerAPI c)
      (do (Coroutine (fn []
                         (CompInitialize c)
                         (.start ^Startable c))
                     cl)
...
(defn- doCljApp ""
  ...
  [ctr opts ^cmzlabsclj.tardis.impl.ext.CljAppMain obj]
  ...
  (let [ ^Properties mf (.getAttr co K_MFPROPS)
    ...
    (let [ svcs (:services env) ]
      (if (empty? svcs)
          ...
          (.reifyServices ^cmzlabsclj.tardis.impl.ext.ContainerAPI co)))
