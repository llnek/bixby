...
(defn- chkManifest
  ...
  [^cmzlabsclj.tardis.core.sys.Element execv
   app
   ^File des
   mf]
...
(defmethod CompInitialize :czc.tardis.impl/Execvisor
  ...
  (let [ ^cmzlabsclj.nucleus.util.core.MubleAPI ctx (.getCtx co)
    ...
    (let [ ^File home (.homeDir ^cmzlabsclj.tardis.impl.exec.ExecvisorAPI co)
...
(defmethod CompInitialize :czc.tardis.impl/BlockMeta
  ...
  (let [ ^URL url (.metaUrl block)
    ...
    (let [ cz (strim (.optString cfg "info" "block-type" ""))
           ^cmzlabsclj.tardis.core.sys.Element co block  ]
      (when (hgl? cz)
