...
(ns ^{ :doc ""
  ...
  (:use [cmzlabsclj.util.core :only [ThrowBadArg MakeMMap notnil? TryC] ])
  (:use [cmzlabsclj.util.str :only [HasAny?] ])
  (:use [cmzlabsclj.util.meta :only [IsBoolean? IsVoid? IsObject?
...
(defn- handleProps ""
  ...
  (with-local-vars [ props (transient {}) ba (transient []) ]
    ...
    (let [ rc (persistent! @props) ]
      (doseq [ [k ^cmzlabsclj.jmx.bean.BPropInfo v] rc ]
  ...
  (let [ impl (MakeMMap) cz (.getClass obj)
    ...
    (reify DynamicMBean
      ...
      (getAttribute [_ attrName]
        (let [ ^cmzlabsclj.jmx.bean.BPropInfo
               prop (get propsMap attrName)
               ^cmzlabsclj.jmx.bean.BFieldInfo
