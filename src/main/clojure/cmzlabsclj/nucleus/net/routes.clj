...
(ns ^{ :doc ""
  ...
  (:use [cmzlabsclj.nucleus.util.core :only [MubleAPI MakeMMap test-nestr] ])
  (:use [cmzlabsclj.nucleus.util.str :only [nsb nichts? hgl?] ])
...
(defn- initRoute ""
  ...
  [^cmzlabsclj.nucleus.util.core.MubleAPI rc
   ^String path]
...
(defn- seek-route ""
  ...
  (if-not (nil? rts)
    (some (fn [^cmzlabsclj.nucleus.net.routes.RouteInfo ri]
...
(defn MakeRouteCracker "Create a url route cracker."
  ...
  ^cmzlabsclj.nucleus.net.routes.RouteCracker
  [routes]
