...
(ns ^{ :doc ""
  ...
  cmzlabsclj.nucleus.jmx.bean

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:use [cmzlabsclj.nucleus.util.core :only [ThrowBadArg MakeMMap notnil? TryC] ])
  (:use [cmzlabsclj.nucleus.util.str :only [HasAny?] ])
  (:use [cmzlabsclj.nucleus.util.meta :only [IsBoolean? IsVoid? IsObject?
  ...
  (with-local-vars [ props (transient {}) ba (transient []) ]
    (doseq [ ^Method mtd (seq mtds) ]
      (let [ mn (.getName mtd)
             ...
             ^cmzlabsclj.nucleus.jmx.bean.BPropInfo
    ...
    (let [ rc (persistent! @props) ]
      (doseq [ [k ^cmzlabsclj.nucleus.jmx.bean.BPropInfo v] rc ]
  ...
  (let [ impl (MakeMMap) cz (.getClass obj)
    ...
    (reify DynamicMBean
      ...
      (getAttribute [_ attrName]
        (let [ ^cmzlabsclj.nucleus.jmx.bean.BPropInfo
               prop (get propsMap attrName)
               ^cmzlabsclj.nucleus.jmx.bean.BFieldInfo
