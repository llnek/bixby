...
(ns ^{ :doc ""
  ...
  (:use [cmzlabsclj.util.core :only [FlattenNil notnil? NowJTstamp nnz] ])
  (:use [cmzlabsclj.util.meta :only [BytesClass CharsClass] ])
  (:use [cmzlabsclj.util.str :only [AddDelim! nsb strim] ])
  (:use [cmzlabsclj.util.io :only [ReadChars ReadBytes ] ])
  (:use [cmzlabsclj.util.dates :only [GmtCal] ])
  (:require [cmzlabsclj.dbio.core
...
(defn MakeProc ""
  ...
  ^cmzlabsclj.dbio.sql.SQLProcAPI
