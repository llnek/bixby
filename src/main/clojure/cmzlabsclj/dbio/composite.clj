...
(ns ^{ :doc ""
  ...
  cmzlabsclj.dbio.composite

  (:require [clojure.tools.logging :as log :only [info warn error debug] ])
  (:require [clojure.string :as cstr])
  (:use [cmzlabsclj.util.core :only [test-nonil notnil? Try!] ])
  (:use [cmzlabsclj.util.str :only [hgl?] ])
  (:require  [cmzlabsclj.dbio.core :as dbcore :only [ese] ])
  (:require [cmzlabsclj.dbio.sql :as dbsql ])
...
(defn- mk-tx ""
  ...
  [^MetaCache metaCache
   ^DBAPI db
   ^cmzlabsclj.dbio.sql.SQLProcAPI proc
   ^Connection conn]
