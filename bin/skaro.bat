...
set SKARO_HOME=%~dp0..
set BINDIR=%~dp0
set PATCHDIR=%SKARO_HOME%\patch\*
set DISTRO=%SKARO_HOME%\dist\exec\*
set BOOT=%SKARO_HOME%\dist\boot\*
set LIBDIR=%SKARO_HOME%\lib\*
...
set BASEDIR=-Dskaro.home=%SKARO_HOME%
set BG=false
set DBGOPTS=
set ECODE=0
set KPORT=4444
set KILLPORT=-Dskaro.kill.port=%KPORT%
set LIBP=-Djava.library.path=$SKARO_HOME/bin
...
set CLDR=-Djava.system.class.loader=com.zotohlab.skaro.loaders.ExecClassLoader
set MAINCZ=czlabclj.tardis.etc.core
...
cd %BINDIR%
:appfg
REM CMDLINE="%JAVA_CMD%" -cp "%BCP%" "%LIBP%" %DBGOPTS% "%LOGREF%" "%NETTY%" "%KILLPORT%" "%BASEDIR%" %CLDR% %MAINCZ% "%SKARO_HOME%" %*
if %BG% == "true" goto runcmd
call :splash
:runcmd
"%JAVA_CMD%" -cp "%BCP%" "%LIBP%" %DBGOPTS% "%LOGREF%" "%NETTY%" "%KILLPORT%" "%BASEDIR%" %CLDR% %MAINCZ% "%SKARO_HOME%" %*
